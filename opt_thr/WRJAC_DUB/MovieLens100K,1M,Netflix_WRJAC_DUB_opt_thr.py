# -*- coding: utf-8 -*-
"""
Created on Thu Jun 22 16:02:18 2023
...
def wrjac_dub(u,v):
    ...
    if len(ind) > 0:
        ...
        cnt2 = sum(np.where((abs((u_m - u[ind]) - (v_m - v[ind])) > td) & (abs((u_m - u[ind]) - (v_m - v[ind])) <= 1.0), True, False))
...
##########################################################################################################################################################
# m -> parameter
td_list = [0.1, 0.3, 0.5, 0.7, 0.9, 1.1]
