# -*- coding: utf-8 -*-
"""
Created on Thu Jun 22 15:27:46 2023
...
def rjac_dub(u,v):
    ...
    if len(ind) > 0:
        ...
        cnt2 = sum(np.where(abs(u[ind] -v[ind]) == 0.0, True, False))
        if abs(u_m - v_m) <= 0.3 or cnt2 == len(ind) :
            cnt3 = 1
        else:
            cnt3 = 0
        return (cnt+cnt3)/len(ind2)
    else:
        return 0
...
elif data_name == 'MovieLens1M': # load and preprocessing
...
elif data_name == 'movietweetings': # load and preprocessing
...
elif data_name == 'filmtrust': # load and preprocessing
...
elif data_name == 'CiaoDVD': # load and preprocessing
...
elif data_name == 'Amazon': # load and preprocessing
    
    data=pd.read_csv('AmazonMovie_small.csv')
    
elif data_name == 'Netflix': # load and preprocessing
...
for td in td_list :
    ...
    #%% 시험결과 저장.
    import datetime
    result.to_csv('result_CiaoDVD/{}_{}_{}_{}_result.csv'.format(data_name,sim_name,td,str(datetime.datetime.now())[:13]+'시'+str(datetime.datetime.now())[14:16]+'분'))
